...
void LoadGame()
{
	...
	if (check((void*)0x748C2B, 0xE8, "Legal info fade-in", true)) {
		nop(0x748C2B, 5); // Legal info fade-in
	}
	else if (check((void*)0x748C7B, 0xE8, "Legal info fade-in", false)) {
		nop(0x748C7B, 5); // Legal info fade-in
	}
	
	dwValue = 1;

	if (check((void*)0x5909AA, 0xBE, "Legal info", false)) {
		patch(0x5909AB, &dwValue, 4);
	}
	...
	if (check((void*)0x748C6B, 0xC6, "Show load game", true)) {
		nop(0x748C6B, 7);
	}
	else if (check((void*)0x748CBB, 0xC6, "Show load game", false)) {
		nop(0x748CBB, 7);
	}

	dwValue = 0x09;
	
	if (check((void*)0x5745DD, 0xC6, "Show load game", false)) {
		patch(0x5745E3, &dwValue, 1);
	}
	
	dwValue = 0x75;

	if (check((void*)0x5737E0, 0x74, "Skip confim", false)) {
		patch(0x5737E0, &dwValue, 1); // Skip confim
	}
...
//	if (MessageBox(0, "Unknown code found", "Fast Loader", MB_YESNO | MB_ICONERROR | MB_DEFBUTTON2) == IDNO) ExitProcess(0);
